...
function Cart() {
  ...
  useEffect(() => {
    ...
    // Instead of fetching, use the imported products data
    let cart_items = products.filter((item) => Object.keys(cart.items).includes(item._id));
    setItems(cart_items);
                ...
                <li className="mb-2" key={item._id}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <img className="h-16" src="/images/peproni.png" alt={item.name} />
